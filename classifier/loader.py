...
from albumentations import (
    ...
    IAAAdditiveGaussianNoise, GaussNoise, MotionBlur, MedianBlur, IAAPiecewiseAffine, VerticalFlip, GaussNoise,
...
class CloudDataset(Dataset):
    def __init__(self, df: pd.DataFrame = None, img_ids: list = [],  datatype: str = 'train'):
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        image_path = os.path.join(self.data_folder, self.img_ids[idx])
        ...
        if self.datatype == 'test':
            ...
            target = torch.tensor(self.df.loc[self.df.Image==self.img_ids[idx]][class_names].values.astype(np.float32)).squeeze()
            return img, target
...
def prepare_df():
    ...
    return train_df, train_ids, valid_ids
...
def get_train_val_loaders(batch_size=16, val_percent=0.1):
    ...
    #print(val_df.head())
    df, train_ids, valid_ids = prepare_df()

    train_dataset = CloudDataset(df=df, img_ids=train_ids, datatype='train')
    valid_dataset = CloudDataset(df=df, img_ids=valid_ids, datatype='valid')
...
def get_test_loader(batch_size=16):
    ...
    test_dataset = CloudDataset(df=None, datatype='test', img_ids=test_ids)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=16)
    test_loader.img_ids = test_ids
